...
IRSettingsDialog::IRSettingsDialog(QWidget* parent) : QDialog(parent), ui(new Ui::IRSettingsDialog)
{
    ...
    ui->rbCompatibilityMode->setChecked(true);




    //ui->cbSerialInterface->setText(cfg.GetQString("DLDI.ImagePath"));
    ui->rbSerialMode->setChecked(false);
    ui->strSerialInterface->setEnabled(false);
    ui->strSerialInterface->setText("/dev/ttyUSB0");


    ui->rbUDPMode->setChecked(false);
    ui->groupBoxUDP->setEnabled(false);

    //Connect fields
    connect(ui->rbCompatibilityMode, &QRadioButton::toggled, this, &IRSettingsDialog::toggleCompatibilitySettings);
    ...
    connect(ui->cbEnablePacketDumps, &QCheckBox::toggled, this, &IRSettingsDialog::togglePacketDumps);

}


void IRSettingsDialog::toggleSerialSettings(bool checked){
    ui->strSerialInterface->setEnabled(checked);
}
void IRSettingsDialog::toggleUDPSettings(bool checked){
    ui->groupBoxUDP->setEnabled(checked);
}

void IRSettingsDialog::toggleCompatibilitySettings(bool checked){

}

void IRSettingsDialog::togglePacketDumps(bool checked){
    ui->pathPacketLog->setEnabled(checked);
}
